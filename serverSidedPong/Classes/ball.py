...
class	Movement() :
	def __init__(self, ballInfo : BallData, racketInfo : list[RacketData], map:Map=Map(), timestamp:float = time.time()) :
		self.ball : BallData = ballInfo
		self.racket1 : RacketData = racketInfo[0]
		self.racket2 : RacketData = racketInfo[1]
		self.ts : float = timestamp
		self.map : Map = myMap

	def calculateLinearMovement(self, vectorMovement : Vector, time : float) -> Point :
		return (Point(self.ball.pos.x + vectorMovement.x * time, self.ball.pos.y + vectorMovement.y * time))

	def isWallIntersection(self, pointTrajectory: Point, timeLeft : float) -> float:
		...
		if (minTimeWallHit != 2.0) :
			...
			self.ball.spd = self.ball.calculateReflexionVector()
			return (timeLeft - minTimeWallHit)
		else :
			self.ball.pos = pointTrajectory
			return (0.0)
		
	def getWallsHit(self) -> None :
		timeLeft : float = 1.0
		while timeLeft > 0.0:
